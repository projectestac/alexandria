...
function xmldb_local_alexandria_upgrade($oldversion) {
    ...
    if ($oldversion < 2021080501) {
        $table = new xmldb_table('local_alexandria_backups');
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('course_id', XMLDB_TYPE_INTEGER, '20', null, XMLDB_NOTNULL, null, null, 'id');
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->add_index('courseid', XMLDB_KEY_UNIQUE, array('course_id'));
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Alexandria savepoint reached.
        upgrade_plugin_savepoint(true, 2021080501, 'local', 'alexandria');
    }
