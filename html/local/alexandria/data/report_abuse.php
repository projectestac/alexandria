...
if ($report) {
    ...
    if (!empty($receiverroles)) {
        $receiverusers = get_role_users(array_keys($receiverroles), $context, true, 'u.*', 'u.id ASC');
    }
...
function data_send_mail($recordid, $user, $fromuser) {
    ...
    if ($fieldid) {
        $info->contentname = $DB->get_field('data_content', 'content', array('fieldid' => $fieldid, 'recordid' => $recordid));
    }

    if ($info->contentname == '') {
        $info->contentname = $CFG->wwwroot . '/mod/data/view.php?rid=' . $recordid;
    }
