    ...
    Y.use('button', 'dd-plugin', 'panel', 'resize', 'moodle-mod_scorm-treeview', function(Y) {
        ...
        var scorm_activate_item = function(node) {
            ...
            if (Y.one('#scorm_object')) {
                ...
                var node_full_url = M.cfg.wwwroot + '/local/alexandria/scorm/loadSCO_preview.php?' + node.title;
            ...
            if (old) {
                if(window_name) {
                    ...
                    scorm_openpopup(M.cfg.wwwroot + "/local/alexandria/scorm/loadSCO_preview.php?" + node.title, window_name, poptions, cwidth, cheight);
                } else {
