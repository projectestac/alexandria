<?PHP  // $Id: view.php,v 1.13 2009/10/23 12:37:11 sarjona Exp $
    ...
    if ($students=jclic_get_students($cm, $course, $jclic->id)){
      foreach($students as $student){
        ...
        if (!$showall || sizeof($sessions)<=0){
          $table_align=$general_align;
          $student_info = print_user_picture($student, $course->id, NULL, 0, true).$student->firstname.' '.$student->lastname;
          jclic_print_row(array ($student_info, !$showall?$starttime:(sizeof($sessions)<=0?'-':'<b>'.$strtotals.'</b>'), '<b>'.$sessions_summary->done.'</b>','<b>'.$sessions_summary->solved.'</b>','<b>'.$sessions_summary->totaltime.'</b>','<b>'.$sessions_summary->score.' %</b>', '<b>'.$sessions_summary->attempts.'</b>'), $table_align);
        }else {
          $table_align=array('center','center','center','center','center','center');
