...
$url = new moodle_url('/mod/scorm/preview.php', array('scoid'=>$scoid, 'cm'=>$cm->id));
if ($mode !== 'normal') {
    $url->param('mode', $mode);
}
if ($currentorg !== '') {
...
add_to_log($course->id, 'scorm', 'view', "preview.php?cm=$cm->id&scoid=$sco->id", "$scorm->id", $cm->id);
...
echo '<script>window.'.$LMS_api.' = 1;</script>';
...
if ($result->prerequisites) {
    if ($scorm->popup != 0 && $displaymode !=='popup') {
        ...
        ?>
            <noscript>
            <!--[if IE]>
                <iframe id="main" class="scoframe" name="main" src="loadSCO_preview.php?id=<?php echo $cm->id.$scoidstr.$modestr; ?>"></iframe>
            <![endif]-->
            <!--[if !IE]>
                <object id="main" class="scoframe" type="text/html" data="loadSCO_preview.php?id=<?php echo $cm->id.$scoidstr.$modestr; ?>"></object>
            <![endif]-->
            </noscript>
        <?php
    }
...
if (empty($scorm->popup) || $displaymode == 'popup') {
    ...
    $PAGE->requires->js_init_call('M.mod_scorm.init_preview', array($result->toctitle, $name, $sco->id, $adlnav));
}
if (!empty($forcejs)) {
