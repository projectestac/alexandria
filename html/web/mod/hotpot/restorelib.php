...
function hotpot_restore_mods($mod, $restore) {
    ...
    if ($data) {
        ...
        //XTEC *************** AFEGIT - Els  HotPot importats sempre estan disponibles
        //2011.02.03 @fcasanel
        $xml['TIMECLOSE']= array('0'=>array("#"=>'0'));
        $xml['TIMEOPEN']= array('0'=>array("#"=>'0'));
        //**************** FI
...
function hotpot_restore_attempts(&$restore, $status, &$xml, &$record, $hotpot_v20=false) {
    ...
    if ($hotpot_v20) {
		...
		//XTEC ************ MODIFICAT - To solve hotpot restore problem
		//2010.07.07 @pferre22
        $more_restore .= 'if (!empty($record->clickreportid) && $record->clickreportid != 0) {';
		//************ ORIGINAL
...
// general purpose function to restore a single record
    ...
    foreach ($table_columns[$table] as $column) {
		...
		//XTEC ************ AFEGIT - To solve restore problem
		//2010.07.07 @pferre22
