<?php  // $Id: field.class.php,v 1.17.2.9 2010/10/25 09:31:10 stronk7 Exp $
///////////////////////////////////////////////////////////////////////////
//                                                                       //
// NOTICE OF COPYRIGHT                                                   //
//                                                                       //
...
class data_field_file extends data_field_base {
    ...
    function display_browse_field($recordid, $template) {
		...
		//XTEC ************ MODIFICAT
                ...
                //If the file is a SCORM (param5), preview-it 2011.05.25 @fcasanel
                if($this->field->param4){
                    ...
                    if($this->field->param5){
                        $str .= '<div id="image" style="display: none;"><iframe id="scorm_preview_iframe" src="'.$CFG->wwwroot.'/mod/scorm/preview.php?a='.$content->content2.'&scoid=0"></iframe><br /><img src="http://alexandria.xtec.cat/pix/i/show.gif" alt="Previsualitza" title="Previsualitza" /><a id="hide_text" href="#foo">'.get_string('hide_prev', 'scorm').'</a></div>';
                    }
                }
		//************ ORIGINAL
		/*
...
#                $course = get_course('course', 'id', $this->data->course);
                ...
                if ($this->field->param5){
                    ...
                    $scorm_record = get_record('data_content', 'fieldid', $this->field->id, 'recordid', $recordid);
                    if ($scorm_record->content2){
                        // Delete old scorm
                        scorm_delete_instance($scorm_record->content2);
                        $relation_id = get_record('course_modules', 'course', '1', 'module', $module_scorm_id, 'instance', $scorm_record->content2);
                        delete_course_module($relation_id->id);
                    }

                    $scorm_id = scorm_add_instance($scorm_object);
                    $scorm_object->instance = $scorm_id;
                    $relation = add_course_module($scorm_object);
