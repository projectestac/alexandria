<?php  // $Id: lib.php,v 1.137.2.58 2010/09/13 08:39:35 andyjdavis Exp $
///////////////////////////////////////////////////////////////////////////
//                                                                       //
// NOTICE OF COPYRIGHT                                                   //
//                                                                       //
...
//2010.08.31
define ('DATA_TIMEADDED', -5);
define ('DATA_RATING', 0);
//************ ORIGINAL
/*
...
// Delete all content associated with the field
    function delete_content($recordid=0) {
        ...
        if ($recordid) {
            //XTEC ************ AFEGIT - If it's a SCORM file delete it
            //2011.05.23 @fcasanel
            if($this->field->param5){
                $fieldid = $this->field->id;
                $scorm_id = get_record('data_content', 'fieldid', $fieldid, 'recordid', $recordid);
                $scorm_id = $scorm_id->content2;
                $module_scorm_id = get_record('modules', 'name', 'scorm');
                $module_scorm_id = $module_scorm_id->id;
                delete_records('scorm', 'id', $scorm_id);
                delete_records('course_modules', 'course', '1', 'module', $module_scorm_id, 'instance', $scorm_id);
            }
...
// Deletes any files associated with this field
    function delete_content_files($recordid='') {
        ...
        //XTEC ************ AFEGIT - If it's a SCORM file delete it
        //2011.05.23 @fcasanel
        if($this->field->param5 && $recordid){
            $fieldid = $this->field->id;
            $scorm_id = get_record('data_content', 'fieldid', $fieldid, 'recordid', $recordid);
            $scorm_id = $scorm_id->content2;
            $scorm_dir = $CFG->dataroot.'/'.$this->data->course.'/'.$CFG->moddata.'/scorm/'.$scorm_id;
            fulldelete($scorm_dir);
        }
        //********** FI
...
function data_print_template($template, $records, $data, $search='', $page=0, $return=false) {
    ...
    // Replacing special tags (##Edit##, ##Delete##, ##More##)
		...
		*/
		...
		//2010.08.31
		/*if ($cm->instance == $CFG->data_coursesdataid){        
            require_once("$CFG->dirroot/blocks/moodleblock.class.php");
            require_once("$CFG->dirroot/blocks/rate_course/block_rate_course.php");
            $bloc_rate= new block_rate_course();
            $patterns[]='##rating##';
            $courseidcontent = get_field('data_content', 'content', 'recordid', $record->id, 'fieldid', $CFG->data_coursefieldid);
            $replacement[] = $bloc_rate->display_rating($courseidcontent, true, false);
        }else{*/
            $patterns[]='##rating##';
            $replacement[] = data_display_rating($data, $record->id);            
		//}
            $patterns[]='##downloads##';
...
//XTEC ************ AFEGIT - Function that returns the number of downloads of courses and data
//2010.08.31

function data_display_rating($data, $recordid){
    global $CFG;
    $ratingsscale = make_grades_menu($data->scale);
    $rating = data_get_ratings_mean($recordid, $ratingsscale);
    $res = '<img src="'.$CFG->wwwroot.'/mod/data/rating_graphic.php?avg='.$rating.'" alt="'.$rating.'" title="'.$rating.'"/>';
    return $res;
}
...
function data_display_downloads($data, $record){
    ...
    if($data->id == $CFG->data_coursesdataid){
       //It's a course
       $courseid = get_field('data_content', 'content', 'fieldid', $CFG->data_coursefieldid, 'recordid', $record->id);
       $downloads = get_field('block_download_course', 'downloading', 'course', $courseid);
       if (empty($downloads)) $downloads = 0;
       return $downloads;
    }else{
       //It's another kind of data
       $maxdownload = get_field('data_content', 'MAX(content4)', 'recordid', $record->id);
       if (empty($maxdownload)) $maxdownload = 0;
       return $maxdownload; 
    }
}
//************ FI
...
function data_print_preference_form($data, $perpage, $search, $sort='', $order='ASC', $search_array = '', $advanced = 0, $mode= '') {
    ...
    $options = array();
	//XTEC ************ AFEGIT - Added new rating criteria for ordering
	//2010.08.31
	//$options[DATA_RATING]    = get_string('rating', 'data');
	//************ FI
...
function data_print_comments($data, $record, $page=0, $mform=false) {
	...
	//XTEC ************ MODIFICAT - Moved new comment at the top of the comments
	//2010.08.31 @pferre22
	echo '<a name="comments"></a><br/>';
	if (isloggedin() && !isguest() && $cancomment) {
...
//XTEC ************ AFEGIT - Get course information from a database record
//2010.08.31
function get_course_information_by_record($rid){
    global $CFG; 
    
    // Course identifier
    $courseid = get_field('data_content', 'content', 'recordid', $rid, 'fieldid', $CFG->data_coursefieldid);

    // Get the course
    $course = get_record('course', 'id', $courseid);

    // Category
    $categorycontent = get_field('data_content', 'content', 'recordid', $rid, 'fieldid', $CFG->data_categoryfieldid);
    $categorycode = "AL";
    if (!isset($categorycontent) || $categorycontent=='') {
      if (record_exists('course_categories', 'id', $CFG->defaultrequestcategory)) {
        $course->category = $CFG->defaultrequestcategory;
      } else{
        $course->category = get_field('course_categories', 'id', 'parent', '0');                          
      }
    } else{
        // Remove area code (Format of category content: XX-Curricular area name)
        $categorycode = substr($categorycontent, 0, 2);
        $categorycontent=substr($categorycontent, 3); 
        
        $course->category = get_field('course_categories', 'id', 'name', $categorycontent);;                        
    }

    // Fullname
    $fullnamerecord = get_record('data_content', 'recordid', $rid, 'fieldid', $CFG->data_fullnamefieldid);
	$fullnamerecord->content = str_replace ( "'","&#039;", $fullnamerecord->content );
    $course->fullname = $fullnamerecord->content;

    // Shortname. Format: AA_MMyy_xxx_W.Z
    $shortnamerecord = get_record('data_content', 'recordid', $rid, 'fieldid', $CFG->data_shortnamefieldid);
    $shortname_ar = explode("_", $shortnamerecord->content);
    $isnew=true;
    if (sizeof($shortname_ar)>=4 && $shortname_ar[0]==$categorycode){
        $creationdate = $shortname_ar[1];
        $totalincategory = $shortname_ar[2];
        $versionname = $shortname_ar[3];        
        $isnew=false;
    }else{
        if (sizeof($shortname_ar)>=4) {
            $coursetime = mktime(0, 0, 0, substr($shortname_ar[1],0,2), 1, "20".substr($shortname_ar[1],2));
            $versionname = $shortname_ar[3];
        } else {
            $coursetime = time();
            $versionname = "1.0";
        } 
        $creationdate = date('my', $coursetime);
        
        //$totalincategory = sprintf('%03s', count_records('course', 'category', $course->category)+1);
        $startmonth = mktime(0, 0, 0, date("m", $coursetime), 1, date("Y", $coursetime));
        $endmonth = mktime(0, 0, 0, date("m", $coursetime)+1, date("d",(mktime(0, 0, 0, date("m", $coursetime)+1, 1, date("Y", $coursetime))))-1, date("Y", $coursetime));
        if ($endmonth==0) $endmonth=time();
        $totalincategory = get_field_sql("SELECT COUNT(*)+1 FROM ".$CFG->prefix."course WHERE category='".$course->category."' AND timecreated>$startmonth AND timecreated<$endmonth ");
        $totalincategory = sprintf('%03s', $totalincategory);
        
    }
    $course->shortname = $categorycode."_".$creationdate."_".$totalincategory."_".$versionname;
    
    $shortnamerecord->content = $course->shortname;
    update_record('data_content', $shortnamerecord);
    
	//$shortnamerecord->content = str_replace ( "'","&#039;", $shortnamerecord->content );
    //$course->shortname = $shortnamerecord->content;
    if ($isnew && record_exists('course', 'shortname', $course->shortname)) {
        $course->shortname.="_".time();
    }

    // Creator & License
    $creatorcontent = get_field('data_content', 'content', 'recordid', $rid, 'fieldid', $CFG->data_creatorfieldid);
    $licenserecord = get_record('data_content', 'recordid', $rid, 'fieldid', $CFG->data_licensefieldid);
    if (empty($licenserecord->content)) {
        $licenserecord->content = $CFG->data_defaultlicense;
        update_record('data_content', $licenserecord);        
    }
    $licensecontent = $licenserecord->content;

    // Summary
    $summaryrecord = get_record('data_content', 'recordid', $rid, 'fieldid', $CFG->data_summaryfieldid);
	$summaryrecord->content = str_replace ( "'","&#039;", $summaryrecord->content );
    $course->summary = $summaryrecord->content."<p> <b>Autor/a:</b> $creatorcontent <br> <b>Llicència ús:</b> $licensecontent </p>";
    return $course;
}
//************ FI

//XTEC ************ AFEGIT - Functions to manage  the download counter
//2010.08.31
function download_info($fieldid, $recordid){
         global $db;
         $object = get_record('data_content', 'fieldid', $fieldid, 'recordid', $recordid);
...
function data_update_downloadings($fieldid, $recordid){
        ...
        $record = get_record('data_content', 'fieldid', $fieldid, 'recordid', $recordid);
        ...
        update_record('data_content', $record);   
        return $value;          
}
//************ FI

//XTEC ************ AFEGIT - Functions to report an abuse of the content
//2010.08.31
