<?php // $Id: questiontypes.class.php,v 1.31.2.37 2009/06/16 13:55:11 joseph_rezeau Exp $
...
//XTEC ************ AFEGIT - File extension (not implemented)
//2010.07.12
define('QUESFILE', 11);
//************ FI
...
$QTYPENAMES = array(
        ...
        QUESDATE =>         'date',
        QUESNUMERIC =>      'numeric',
        //******* AFEGIT XTEC  - File extension (not implemented)
        //2010.07.12
        QUESFILE =>         'file',
        //************ FI
...
/// appropriate function based on its table.
    ...
    function insert_response_rank($rid, $formdata) {
        if($this->type_id == 8) { // Rank
            ...
            $r = strtolower($formdata->{'q'.$this->id});
...
/// appropriate function based on its table.
    ...
    function get_response_text_results($rids = false) {
        ...
        //XTEC ************ MODIFICAT - Oracle clob problem
		//2010.07.12
		$sql = 'SELECT TO_CHAR(response) AS response, COUNT(response_id) AS num '.
               'FROM '.$CFG->prefix.'questionnaire_'.$this->response_table.' '.
               'WHERE question_id='.$this->id.$ridstr.' '.
               'GROUP BY TO_CHAR(response)';
		//************ ORIGINAL
		/*$sql = 'SELECT response, COUNT(response_id) AS num '.
               'FROM '.$CFG->prefix.'questionnaire_'.$this->response_table.' '.
               'WHERE question_id='.$this->id.$ridstr.' '.
               'GROUP BY response';*/
		//************ FI
    ...
    function get_response_single_results($rids=false) {
        ...
        //XTEC ************ MODIFICAT - Oracle clob problem
		//2010.07.12 @pferre22
		$sql = 'SELECT qc.id, TO_CHAR(qc.content) as content, COUNT(rt.response_id) AS num '.
               ...
               'GROUP BY qc.id,TO_CHAR(qc.content) ' .
               'ORDER BY qc.id';
		//************ ORIGINAL
        ...
        //XTEC ************ MODIFICAT - Oracle clob problem
		//2010.07.12 @pferre22
		$sql = 'SELECT TO_CHAR(rt.response) as response, TO_CHAR(qc.content) as content , COUNT( rt.response_id ) AS num '.
               ...
               'GROUP BY TO_CHAR(rt.response),TO_CHAR(qc.content),qc.id ' .
               'ORDER BY qc.id';
		//************ ORIGINAL
		/*$sql = 'SELECT rt.response, qc.content , COUNT( rt.response_id ) AS num '.
               'FROM '.$CFG->prefix.'questionnaire_response_other rt, '.
                       $CFG->prefix.'questionnaire_quest_choice qc '.
               'WHERE rt.question_id='.$this->id.' AND rt.choice_id=qc.id'.$ridstr.' '.
               'GROUP BY rt.response,qc.content,qc.id ' .
               'ORDER BY qc.id';*/
		//************ FI
    ...
    function get_response_rank_results($rids=false) {
        ...
        if($this->type_id  == 8) { //Rank
         // JR there can't be an !other field in rating questions ???
            ...
            //XTEC ************ MODIFICAT - Oracle clob problem
			//2010.07.12 
			$sql = 'SELECT TO_CHAR(C.content) AS content, AVG(A.rank+1) AS average '.
...
/// Display Methods
    ...
    function display_results($rids=false, $guicross=false) {

        $method = 'display_'.$this->response_table.'_results';
        if (method_exists($this, $method)) {
            $a = $this->$method($rids, $guicross);
            echo ('<hr />');
            return $a;
        } else {
            return false;
    ...
    function display_response_text_results($rids = false) {
        ...
        if ($rows = $this->get_response_text_results($rids)) {
            foreach ($rows as $row) {
                ...
                $count = $row->num;
                if(!empty($this->text)) {
                    $this->counts[clean_text($this->text)] = $count;
                }
            }
    ...
    function display_resp_multiple_results($rids=false, $guicross=false) {
        //XTEC ************ AFEGIT
		//2010.07.12 
        echo clean_text($this->content).'</div>';
        //************ FI
        ...
        if ($rows = $this->get_response_multiple_results($rids)) {
            ...
            $this->mkrespercent(count($rids), $this->precise, 0, $guicross);
        } else {
            print_string('noresponsedata', 'questionnaire');
        }
    }

    function display_response_rank_results($rids=false) {
        //XTEC ************ AFEGIT
		//2010.07.12 
        echo clean_text($this->content).'</div>';
        //************ FI
        ...
        if ($rows = $this->get_response_rank_results($rids)) {
            if($this->type_id == 8) { //Rank
                ...
                $this->mkresavg(count($rids), $this->precise, $prtotal, $this->length);
        ...
        <div class="qnOuter">
            <table class="qnInnerTable" style="width:100%" cellpadding="10"  cellspacing="1">
                <tr>
                    ...
                    if ($this->required == 'y') {
						//XTEC ************ MODIFICAT - Put * insteadof image
						//2010.07.12 
    ...
    function drop_survey_display($data) { // Drop
        ...
        foreach ($this->choices as $id => $choice) {
            ...
            //$options[$id] = format_text($content, FORMAT_HTML);
			//************ FI
        }
	...
	//XTEC ************ AFEGIT - File extension (not implemented)
	//2010.07.12
	function file_survey_display($data) { // File
        echo '<input type="file" size="'.$this->length.'" name="q'.$this->id.'"'.
             ($this->precise > 0 ? ' maxlength="'.$this->precise.'"' : '').' value="'.
             (isset($data->{'q'.$this->id}) ? stripslashes($data->{'q'.$this->id}) : '').'" />';
    }
	//************ FI
	...
	//XTEC ************ MODIFICAT - Improvement
	//2010.07.12 
	echo '
    <div class="questioncontainer sectiontext">
        <div class="qnOuter">
            <table class="qnInnerTable" style="width:100%" cellpadding="10"  cellspacing="1">
                <tr>
    ...
    //XTEC ************ AFEGIT - File extension (not implemented)
	//2010.07.12
	function file_response_display($data) {
...
///****
    ...
    function mkrespercent($total, $precision, $showTotals, $guicross=false) {
        global $CFG;
        $precision = 0;
        $i=0;
        $alt = '';
    ...
    function mkreslistdate($total, $precision, $showTotals) {
        ...
        if (!empty($this->counts) && is_array($this->counts)) {
            ...
            while(list($text,$num) = each($this->counts)) {
                $text = userdate ( $text, $dateformat, '1');    // change timestamp into readable dates
                $table->data[] = array($num, $text);
            }
        } else {
            $table->data[] = array('', get_string('noresponsedata', 'questionnaire'));
