<?php  // $Id: lib.php,v 1.538.2.82 2010/10/13 20:30:23 skodak Exp $
...
// Returns a number of useful structures for course displays
  	//XTEC ************ AFEGIT -  Only EOI can install, upgrade or use eoicampus module
	//2010.06.30
	global $CFG;
	//************ FI
    ...
    if ($allmods = get_records("modules")) {
        foreach ($allmods as $mod) {
        	//XTEC ************ MODIFICAT - Only EOI can install, upgrade or use eoicampus module
			//2010.06.30
            if ($mod->visible && ($CFG->iseoi || $mod->name!='eoicampus')) {
			//************ ORIGINAL 
            //if ($mod->visible) {
			//************ FI
...
function print_course($course, $highlightterms = '') {
	...
	//XTEC ************ AFEGIT - Added course ratings information
	//2010.08.05
    require_once("$CFG->dirroot/blocks/moodleblock.class.php");
    require_once("$CFG->dirroot/blocks/rate_course/block_rate_course.php");
    $bloc_rate= new block_rate_course();
    echo "<br><div class=\"teachers\">".$bloc_rate->display_rating($course->id, true, false)."</div>";  
	//************ FI
...
function print_course_search($value="", $return=false, $format="plain") {
    ...
    if ($format == 'plain') {
        ...
        $output .= '<input type="submit" value="'.get_string('go').'" />';
        $output .= '</fieldset></form>';
	//XTEC ************ AFEGIT - Show shortest bar into the topmenu bar
	//2010.08.05
...
function delete_course_module($id) {
    ...
    if ($grade_items = grade_item::fetch_all(array('itemtype'=>'mod', 'itemmodule'=>$modulename,
        ...
        //XTEC ************ AFEGIT - Delete image (if it exists)
        //2011.04.15 @jfern343 (by @pferre22) https://projectes.lafarga.cat/projects/agora/tracker/841/1737/detail
	require_once('format/senzill/lib.php');
	senzill_delete_module_image($cm->course, $id);
	//************ FI
