...
/// Check if the user has actually submitted login data to us
    ...
    if (empty($CFG->usesid) and $testcookies and (get_moodle_cookie() == '')) {    // Login without cookie when test requested
        ...
        /// Prepare redirection
			//XTEC ************ AFEGIT - Check if the user agreed the conditions of usage and take out ldap data to complete user profile
			//2010.08.31
			/*TESTING*/
            /*$ldap = new auth_plugin_ldap();           
            $array = $ldap->get_userinfo($user->username);
            print_r($array); die();*/
			if (user_not_policy_agreed($USER) && ($USER->username != 'guest')) {
                $urltogo = $CFG->wwwroot.'/user/policy.php';
