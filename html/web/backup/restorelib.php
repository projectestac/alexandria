...
define('RESTORE_GROUPS_GROUPINGS', 3);
    ...
    function restore_read_xml_blocks ($restore, $xml_file) {
		//XTEC ************ AFEGIT - Blocks changed for restore
		//2010.08.05 @pferre22
        $doc = new DOMDocument();
        $doc->load($xml_file);
        $doc->formatOutput = true;
        $blocks_str ="<BLOCKS>";
        //$blocks_str.=" <BLOCK><ID>1</ID><NAME>rate_course</NAME><PAGEID>3</PAGEID><PAGETYPE>course-view</PAGETYPE><POSITION>l</POSITION><WEIGHT>0</WEIGHT><VISIBLE>1</VISIBLE><CONFIGDATA>Tjs=</CONFIGDATA><ROLES_OVERRIDES></ROLES_OVERRIDES><ROLES_ASSIGNMENTS></ROLES_ASSIGNMENTS></BLOCK>";
        //$blocks_str.=" <BLOCK><ID>2</ID><NAME>version_course</NAME><PAGEID>3</PAGEID><PAGETYPE>course-view</PAGETYPE><POSITION>l</POSITION><WEIGHT>1</WEIGHT><VISIBLE>1</VISIBLE><CONFIGDATA>Tjs=</CONFIGDATA><ROLES_OVERRIDES></ROLES_OVERRIDES><ROLES_ASSIGNMENTS></ROLES_ASSIGNMENTS></BLOCK>";
        //$blocks_str.=" <BLOCK><ID>3</ID><NAME>download_course</NAME><PAGEID>3</PAGEID><PAGETYPE>course-view</PAGETYPE><POSITION>l</POSITION><WEIGHT>2</WEIGHT><VISIBLE>1</VISIBLE><CONFIGDATA>Tjs=</CONFIGDATA><ROLES_OVERRIDES></ROLES_OVERRIDES><ROLES_ASSIGNMENTS></ROLES_ASSIGNMENTS></BLOCK>";
        //$blocks_str.=" <BLOCK><ID>4</ID><NAME>share</NAME><PAGEID>3</PAGEID><PAGETYPE>course-view</PAGETYPE><POSITION>l</POSITION><WEIGHT>3</WEIGHT><VISIBLE>1</VISIBLE><CONFIGDATA>Tjs=</CONFIGDATA><ROLES_OVERRIDES></ROLES_OVERRIDES><ROLES_ASSIGNMENTS></ROLES_ASSIGNMENTS></BLOCK>";
        $blocks_str.="</BLOCKS>";
        $blocks_doc = new DOMDocument();
        $blocks_doc->loadXML($blocks_str);
        
        $blocks_node = $doc->importNode($blocks_doc->getElementsByTagName("BLOCKS")->item(0), true);
        if (count($doc->getElementsByTagName("BLOCKS")->item(0)) == 0 ){            
            // Create new node
            $old_blocks_node = $doc->getElementsByTagName("COURSE")->item(0)->appendChild($doc->createElement("BLOCKS"));
        } else {
            $old_blocks_node = $doc->getElementsByTagName("BLOCKS")->item(0);
        }
        $old_blocks_node->parentNode->replaceChild($blocks_node, $old_blocks_node);
        $doc->save($xml_file);
        //************ FI
    ...
    */
    ...
    function restore_split_xml ($xml_file, $preferences) {
        ...
        if ($status) {
            ...
            while ($data = fread($fp, 8192)) {
                ...
                //XTEC ************ AFEGIT - Fix XML encoding errors
				//2010.07.19 @pferre22 
                if(!mb_check_encoding($data,'UTF-8')){
					$data = mb_convert_encoding($data, 'UTF-8');
					if(!mb_check_encoding($data,'UTF-8')){
						$array = preg_split('//', $data, -1, PREG_SPLIT_NO_EMPTY);
						foreach($array as $i => $cha){
							if(ord($cha) > 200){
								$cha = iconv("ISO-8859-1//TRANSLIT","UTF-8",$cha);
								$data = utf8_encode(substr_replace($data,$cha,$i,1));
    ...
    function restore_execute(&$restore,$info,$course_header,&$errorstr) {
        ...
        if ($restore->restoreto == RESTORETO_NEW_COURSE) {
            ...
            $course = get_record("course","id",$restore->course_id);
            if ($course) {
				//XTEC ************ AFEGIT - Update correct sections number
				//2010.08.05
                $course->numsections = $course_header->course_numsections;
                if (defined('RESTORE_SILENTLY') && !update_record('course', $course)){
                    $errrostr = "An error occurred while updating some of the course contents.";
                    return false;                   
                }
				//************ FI
                if (!defined('RESTORE_SILENTLY')) {
