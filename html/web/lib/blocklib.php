...
function blocks_have_content(&$pageblocks, $position) {
    ...
    //XTEC ************ AFEGIT - To hide blocks for students
	//2010.06.30 @pferre22
    global $COURSE, $CFG;
    $coursecontext = get_context_instance(CONTEXT_COURSE, $COURSE->id);
    $managecourseblocks = has_capability('moodle/site:manageblocks', $coursecontext) || has_capability('moodle/grade:edit', $coursecontext);  	
    $hideblocks = (isset($COURSE->showblocks) && $COURSE->showblocks == 0 && isset($CFG->hideblockstostudents) && $CFG->hideblockstostudents)|| (isset($COURSE->showblocks) && $COURSE->showblocks == 2);
    if(!$managecourseblocks && $hideblocks)
		return false;
    //************ FI
    
    //XTEC ************ MODIFICAT - To solve a problem with Turn editing on
	//2010.07.12 @pferre22 - http://moodle.org/mod/forum/discuss.php?d=140622
...
function upgrade_blocks_plugins($continueto) {
    ...
    foreach ($blocks as $blockname) {
        ...
        if ($currblock = get_record('block', 'name', $block->name)) {
			...
			//XTEC ************ AFEGIT - It's necessary to hide admin_service block (it can be used only from the portal)
			//2010.07.01
        	if ($CFG->isportal || $block->name != 'admin_service' ){
            //************ FI
