<?PHP  //$Id: upgrade.php,v 1.154.2.79 2011/02/02 13:15:23 moodlerobot Exp $
...
// add forgotten table
    ...
    /// Copy data from old field to new field
		...
		//XTEC ************ MODIFICAT - Error updating
		//2010.07.01
        execute_sql('UPDATE '.$CFG->prefix.'role_names SET name = text');
		//************ ORIGINAL
    ...
    if ($result && $oldversion < 2007101561.02) {
		//XTEC ************ MODIFICAT - Do not email administrators about the upgrade
		$messagesubject = s($SITE->shortname).": Actualització de l'espai Moodle d'Àgora";
        $message  = '<p>El vostre espai Moodle (<a href="'.s($CFG->wwwroot).'" target="_blank">'.s($CFG->wwwroot).'</a>) s\'ha actualitat a la versió 1.9.9. Podeu consultar els detalls de les millores que inclou aquesta versió a <a href="http://docs.moodle.org/en/Moodle_1.9.9_release_notes" target="_blank">la pàgina oficial de Moodle</a> (en anglès).</p>';
		$message  .= '<p>Heu provat ja l\'aparença <b>XTEC2</b>? Es tracta d\'un tema nou que permet personalitzar de forma molt senzilla alguns aspectes de disseny, com ara els colors, la mida de la lletra, el joc d\'icones o imatge de la capçalera. Si encara no us heu passat a l\'aparença nova podeu fer-ho des del <a href="'.s($CFG->wwwroot).'/theme/index.php" title="Selector de temes"><b><u>Selector de temes</u></b></a>. ';
		$message  .= 'Trobareu més informació sobre les novetats del tema XTEC2 a <a href="http://agora.xtec.cat/moodle/moodle/mod/glossary/view.php?id=461&mode=entry&hook=581" title="PMF" target="_blank"><u>aquesta pregunta freqüent</u></a>.</p>';
		$message  .= 'Cordialment,<br>Equip Àgora';
		//************ ORIGINAL
        ...
        foreach ($adminusers as $adminuser) {
			//XTEC ************ ELIMINAT - Changing administrators password would cause confussion among users
            ...
            */
            //************ FI
            //XTEC ************ AFEGIT - Do not email and change settings of xtecadmin
			if($adminuser->username != 'xtecadmin'){
			//************ FI
    ...
    //XTEC ************ AFEGIT - Add new parameters to course
	//2010.08.04 @pferre22
    if ($result && $oldversion < 2007101590.01) {
 		$table = new XMLDBTable('course');
 		
    /// Define field navigationbar to be added to course
        $field = new XMLDBField('navigationbar');
        $field->setAttributes(XMLDB_TYPE_INTEGER, '2', XMLDB_UNSIGNED, null, null, null, null, '0', 'defaultrole');
    /// Launch add field navigationbar
        $result = $result && add_field($table, $field);

    /// Define field gradesonnav to be added to course
        $field = new XMLDBField('gradesonnav');
        $field->setAttributes(XMLDB_TYPE_INTEGER, '2', XMLDB_UNSIGNED, null, null, null, null, '1', 'navigationbar');

    /// Launch add field gradesonnav
        $result = $result && add_field($table, $field);
        
    /// Define field showblocks to be added to course
        $field = new XMLDBField('showblocks');
        $field->setAttributes(XMLDB_TYPE_INTEGER, '2', XMLDB_UNSIGNED, null, null, null, null, '0', 'gradesonnav');

    /// Launch add field showblocks
        $result = $result && add_field($table, $field);
    ...
    //XTEC ************ AFEGIT - More new parameters to course
    //2011.02.09 @jfern343
        if ($result && $oldversion < 2007101590.02) {
                    $table = new XMLDBTable('course');

        /// Define field topicsonnav to be added to course
            $field = new XMLDBField('topicsonnav');
            $field->setAttributes(XMLDB_TYPE_INTEGER, '1', XMLDB_UNSIGNED, null, null, null, null, '1', 'showblocks');
        /// Launch add field topicsonnav
            $result = $result && add_field($table, $field);

        /// Define field topicslistonnav to be added to course
            $field = new XMLDBField('topicslistonnav');
            $field->setAttributes(XMLDB_TYPE_INTEGER, '1', XMLDB_UNSIGNED, null, null, null, null, '1', 'topicsonnav');

        /// Launch add field topicslistonnav
            $result = $result && add_field($table, $field);
    ...
    //XTEC ************ MODIFICAT - Revised version number because we have "added" two extra Moodle versions
    //2011.06.10 @sarjona
    if ($result && $oldversion < 2007101590.03) {
        ...
        upgrade_main_savepoint($result, 2007101590.03);
    }
    //************ ORIGINAL
    /*        
    if ($result && $oldversion < 2007101590.01) {
