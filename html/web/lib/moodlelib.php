...
/// Make sure the USER has a sesskey set up.  Used for checking script parameters.
	...
	//XTEC ************ MODIFICAT - It's not necessary that the guest user accept policy agree
	//2010.08.30 
	        if (!$USER->policyagreed && ($USER->username != 'guest')) {
	//************ ORIGINAL
	/*
...
//XTEC ************ AFEGIT - Determines if a user agreed the conditions of usage
//2010.08.30
/**
 * Determines if a user agreed the conditions of usage
 *
 * @param user $user 
 * @return bool
 */
function user_not_policy_agreed($user) {
    return ($user->username != 'guest' and (empty($user->policyagreed)));
}
//************ FI
...
function create_user_record($username, $password, $auth='manual') {
	...
	//XTEC ************ MODIFICAT - The function needs an username and a password to read ldap
	//2010.07.07 @fcasanel
    if ($newinfo = $authplugin->get_userinfo($username, $password)) {
	//************ ORIGINAL
    ...
    if (!empty($newuser->firstname)){
    	...
    	$newuser->firstname = str_replace  (array('Ñ','Ç'),array('ñ','ç'),$newuser->firstname);
    	$newuser->firstname = ucwords($newuser->firstname);
    }
    if (!empty($newuser->lastname)){
        $newuser->lastname = strtolower($newuser->lastname);
    	$newuser->lastname = str_replace  (array('Ñ','Ç'),array('ñ','ç'),$newuser->lastname);
    	$newuser->lastname = ucwords($newuser->lastname);
...
/// We are going to use textlib services here
    ...
    if (is_string($from)) { // So we can pass whatever we want if there is need
        ...
        //XTEC ************** AFEGIT - Replyadress is always required
        //2011.05.13 @fcasanel
        if (empty($replyto)) {
            $mail->AddReplyTo($CFG->noreplyaddress);
        }
        //************** FI
        ...
        //XTEC ************ AFEGIT - To avoid replying to SMTP address when sending via Google Apps
        //2010.10.19 @pferre22
        if (empty($replyto)) {
            $mail->AddReplyTo($CFG->noreplyaddress);
        }
	...
	//XTEC ************ MODIFICAT - To identify the messages from Alexandria service
	//2011.05.04 @fcasanellas
        $mail->Subject = "[Alexandria] ".substr(stripslashes($subject), 0, 900);
	//************ ORIGINAL
...
/// encoding to the specified one
    ...
    //XTEC ************ MODIFICAT - To use apligest system to send mails if it's necessary
    //28.04.2011 @fcasanel
    if($CFG->apligestmail){
        require_once ($CFG->libdir.'/mailer/message.class.php');
        require_once ($CFG->libdir.'/mailer/mailsender.class.php');
        ...
        $aplication =  $CFG->apligestaplic;
        $replyto = $mail->ReplyTo[0][0];
        $sender = new mailsender($aplication, $replyto, 'educacio', $environment, $log, $logdebug, $logpath );
...
function setnew_password_and_mail($user) {
	...
	//XTEC ************* MODIFICAT - Supportname in the subject
    //2011.05.04 @fcasanel
    $subject  = format_string($CFG->supportname) .': '. get_string('newusernewpasswordsubj');
	//************** ORIGINAL
	//$subject  = format_string($site->fullname) .': '. get_string('newusernewpasswordsubj');
	//************** FI

    //XTEC ************* MODIFICAT - Correu de suport només s'utilitza en el camp "Reply to"
    //2010.12.15 @Fèlix Casanellas
    $from = new object();
    $from->email = $CFG->smtpuser;
    $from->firstname = $supportuser->firstname;
    $from->lastname = $supportuser->lastname;
    $from->maildisplay = $supportuser->maildisplay;
    return email_to_user($user, $from, $subject, $message,'','','',true,$supportuser->email,fullname($from));
    //************** ORIGINAL
...
function reset_password_and_mail($user) {
	...
	//XTEC ************* MODIFICAT - Supportname in the subject
    //2011.05.04 @fcasanel
    $subject  = format_string($CFG->supportname) .': '. get_string('changedpassword');
	//************** ORIGINAL
 ...
 */
 function send_confirmation_email($user) {
	...
	//XTEC ************* MODIFICAT - Supportname in the subject
    //2011.05.04 @fcasanel
    $subject = get_string('emailconfirmationsubject', '', format_string($CFG->supportname));
	//************** ORIGINAL
    ...
    $from->lastname = $supportuser->lastname;
    $from->maildisplay = $supportuser->maildisplay;
    return email_to_user($user, $from, $subject, $message, $messagehtml,'','',true,$supportuser->email,fullname($from));
    //************** ORIGINAL
...
function send_password_change_confirmation_email($user) {
	...
	//XTEC ************* MODIFICAT - Supportname in the subject
    //2011.05.04 @fcasanel
    $subject = get_string('emailpasswordconfirmationsubject', '', format_string($CFG->supportname));
	//************** ORIGINAL
...
function send_password_change_info($user) {
    ...
    if (!is_enabled_auth($user->auth) or $user->auth == 'nologin') {
		...
		//XTEC ************* MODIFICAT - Supportname in the subject
		//2011.05.04 @fcasanel
		$subject = get_string('emailpasswordchangeinfosubject', '', format_string($CFG->supportname));
		//************** ORIGINAL
...
/// Define the two or three major locations of language strings for this module
    ...
    if (isset($CFG->running_installer)) {
		...
		//XTEC ************ ELIMINAT - To solve problem with HTML Editor
		//2010.09.02
		/*    	
        $locations[] = $CFG->dataroot.'/lang/';
		*/
...
//Produces the same result as info-zip unzip.
    ...
    //XTEC ************ AFEGIT - To solve problems unzipping files with special chars
	//2010.07.07
