...
/// to randomly choose the percentage of times we should run these jobs.
    ...
    //XTEC ************ AFEGIT - Comprova si s'ha de fer un backup dels cursos i si és així fa previament un reset
    //2010.12.02 @Fèlix Casanellas    
    mtrace("Reset courses... \n");
    global $CFG;
    // Select all courses except the main page one (which has the category=0)
    $courses = get_recordset_sql("SELECT * FROM {$CFG->prefix}course WHERE category!=0");
    ...
    $data->reset_jclic_deleteallsessions = '1';
    $data->reset_quiz_attempts = '1';
    $data->submitbutton = 'Reinicia curs';
    $data->reset_start_date_old = '0';

    while ($course = rs_fetch_next_record($courses)) {
    //Elimina el valor del camp password de la base de dades dels cursos    
    //2011.03.31 @Aida Regi
        $course->password='';
        $data->password =$course->password;
        $data->id = $course->id;
        update_course($data);
        $nextstarttime = get_field("backup_courses","nextstarttime", null, null);
        if (!isset($nextstarttime) || time() >= $nextstarttime){
            $status = reset_course_userdata($data);
            mtrace('--'.$course->fullname.'('.$course->id.'):');
            foreach ($status as $component){
                mtrace('    -'.$component['component'].':'.$component['item'].$component['error']);
            }
            mtrace("\n");
        }
    }
