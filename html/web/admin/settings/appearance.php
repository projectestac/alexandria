...
if ($hassiteconfig) { // speedup for non-admins, add all caps used on this page
	...
	//XTEC ************ AFEGIT - Edition of top menu option 
	//2010.08.05
    // "thememenusettings" settingpage
    $temp = new admin_settingpage('thememenusettings', get_string('thememenusettings', 'admin'));
    $temp->add(new admin_setting_configtext('thememenuoptions', get_string('thememenuoptions', 'admin'), get_string('configthememenuoptions','admin'), '', PARAM_NOTAGS));
    $temp->add(new admin_setting_configtext('thememenuconditions', get_string('thememenuconditions', 'admin'), get_string('configthememenuconditions','admin'), '#', PARAM_URL));
    $temp->add(new admin_setting_configtext('thememenuabout', get_string('thememenuabout', 'admin'), get_string('configthememenuabout','admin'), '#', PARAM_URL));
    ...
    //XTEC ************ AFEGIT - to add the feature of hide blocks to students
    //2010.06.30 @pferre22
    $temp->add(new admin_setting_configcheckbox('hideblockstostudents', get_string('hideblockstostudents', 'admin'), get_string('confighideblockstostudents', 'admin'), 0));
    //************ FI
    ...
    //XTEC ************ AFEGIT - To change specific config settings
    //2010.06.30 @pferre22
    global $THEME; 
    if(isset($THEME->configurable) && $THEME->configurable){
    	$ADMIN->add('themes', new admin_externalpage('themeconfig', get_string('themeconfig','theme_config'), $CFG->wwwroot . '/theme/'.current_theme().'/themeconfig.php'));
    }
    //************ FI
