...
/// Check to see if anything else needs to be done with it
    ...
    if ($emailforced || (!isset($preference->message_emailmessages) || $preference->message_emailmessages)) {  // Receiver wants mail forwarding
        ...
        if ($emailforced || (time() - $userto->lastaccess) > ((int)$preference->message_emailtimenosee * 60)) { // Long enough
            ...
            //XTEC ***************** MODIFICACIO - Personal messages sent from school account. The name of the autor appears on the subject
            //2011.02.16 @fcasanel
            if($CFG->apligestmail == '0') $userfrom = $SITE->fullname;
            $messagesubject = get_string('messagefrom', 'message').$USER->firstname." ".$USER->lastname;

            if ($USER->emailstop){
                //Email adress disabled
                $replyto = $CFG->noreplyaddress;
            }else{
                //Email adress enabled
                if ($USER->maildisplay == 0){
                    //Hide for everyone
                    $replyto = $CFG->noreplyaddress;
                }else if ($USER->maildisplay == 1){
                    //Allow for everyone
                    if($CFG->apligestmail){
                            $replyto = $USER->email;
                        }else{
                            $replyto = $USER->firstname." ".$USER->lastname." <".$USER->email.">";
                        }
                }else{
                    //Allow only other course members
                    if (isadmin($USER->id)){
                        //Is admin (member of all courses)
                        if($CFG->apligestmail){
                            $replyto = $USER->email;
                        }else{
                            $replyto = $USER->firstname." ".$USER->lastname." <".$USER->email.">";
                        ...
                        $userto_courses = get_my_courses($userto->id);
                        $find = 0;
                        foreach($user_courses as $course_id => $course){
                            if(in_array($course_id, array_keys($userto_courses))){
                                $find = 1;
                                break;
                            }
                        }
            ...
            if (email_to_user($userto, $userfrom, $messagesubject, $messagetext, $messagehtml, '', '', true, $replyto)) {
            // **************** ORIGINAL
